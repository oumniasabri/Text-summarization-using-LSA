...
class Window(QMainWindow, QWidget):
    ...
    def init_ui(self):
        ...
        self.num_lines = QLabel(self, text='Number of lines:')
    ...
    def summarize(self):
        # noinspection PyBroadException
        try:
            self.n = self.line_num_input.text()
            self.n = (int(self.n))
            self.my_text = self.textbox_input.toPlainText()
            if self.n > len(sent_tokenize(self.my_text)):
                QMessageBox.warning(self, 'Error!', self.warning_text_1,QMessageBox.Ok)
            else:
                self.my_text = self.textbox_input.toPlainText()
                self.my_output = summarizer.summarizer(doc=self.my_text,k=self.n)
            ...
            QMessageBox.warning(self, 'Error!', 'You have to input the text in the input textbox!\n'
                                                'You have to input a number in the number box!',QMessageBox.Ok)
    ...
    def file_save(self):
        try:
            ...
            print(e)
            QMessageBox.warning(self, 'Error!', "You don't have any text to save!",QMessageBox.Ok)
